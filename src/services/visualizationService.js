...
async function createSajuVisualization(input) {
  ...
  if (birthDateTime) {
    ...
    analysisDetails = {
      ...
      core_events: sajuComputed.predictedEvents.map((e) => ({
        current: e.current,
        target: e.target,
        ...
        detail: e.detail,
        tier: e.tier,
        tier_label: e.tierLabel,
        description: e.description,
      })),
      timing_events: sajuComputed.timingEvents.map((e) => ({
        current: e.current,
        target: e.target,
        currentHanja: e.currentHanja,
        targetHanja: e.targetHanja,
    ...
    if (!analysisSummary) {
      ...
      if (geminiNarrative) {
        ...
        // 2차(대체): Gemini 키가 없거나 호출이 실패하면 규칙 기반 문장으로 대체
